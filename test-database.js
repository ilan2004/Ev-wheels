  ...
  try {
    ...
    if (customersError) {
      ...
      customers?.forEach(customer => {
        console.log(`  - ${customer.id} | ${customer.name} | ${customer.contact}`);
      });
    }
    ...
    if (batteriesError) {
      ...
      batteries?.forEach(battery => {
        console.log(`  - ${battery.id} | ${battery.serial_number} | ${battery.brand} | ${battery.status}`);
      });
      ...
      if (batteries && batteries.length > 0) {
        ...
        if (detailError) {
          console.error('❌ Detailed battery fetch error:', detailError.message);
        } else {
          console.log('✅ Detailed battery fetch successful');
          console.log('   Customer:', detailedBattery.customer?.name || 'Not found');
        }
      }
    }
    ...
    if (!customers?.length && !batteries?.length) {
      ...
      if (tablesError) {
        ...
        console.log('Tables that exist:', tablesData?.map(t => t.table_name) || []);
      }
    }
