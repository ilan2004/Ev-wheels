  ...
  try {
    ...
    async function upsertUser(u) {
      ...
      const { error: pErr } = await admin
        .from('profiles')
        .upsert(
          { user_id: userId, username: u.username, email: u.email },
          { onConflict: 'user_id' }
        );
      if (pErr) throw pErr;
      // role
    ...
    const custByLoc = {};
    for (const code of locationCodes)
      custByLoc[code] = await seedCustomers(code, 10);
    ...
    async function seedBatteries(locCode, customerIds, n = 10) {
      ...
      const rows = Array.from({ length: n }).map(() => ({
        ...
        battery_type: faker.helpers.arrayElement([
          'li-ion',
          'lfp',
          'nmc',
          'other'
    ...
    const batByLoc = {};
    for (const code of locationCodes)
      batByLoc[code] = await seedBatteries(code, custByLoc[code], 10);
    ...
    const ticketsByLoc = {};
    for (const code of locationCodes)
      ticketsByLoc[code] = await seedTickets(code, custByLoc[code], 8);
    ...
    async function seedVehicleCases(locCode, ticketIds, customerIds, n = 5) {
      ...
      const { data: tkRows } = await admin
        .from('service_tickets')
        .select('id')
        .in('id', ticketIds.slice(0, n));
    ...
    const invoicesByLoc = {};
    for (const code of locationCodes)
      quotesByLoc[code] = await seedQuotes(code, custByLoc[code], 4);
    for (const code of locationCodes)
      invoicesByLoc[code] = await seedInvoices(code, custByLoc[code], 4);
    ...
    async function seedMovements(locCode, n = 5) {
      ...
      if (error)
        console.warn('inventory_movements insert warn:', error.message);
    }
