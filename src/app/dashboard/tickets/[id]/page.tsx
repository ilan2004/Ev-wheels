...
export default function TicketDetailPage() {
  ...
  const [attSort, setAttSort] = useState<'newest'|'oldest'|'size'>('newest');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const a of attachments) {
        const bucket = a.attachment_type === 'audio' ? 'media-audio' : 'media-photos';
        try {
  ...
  const uploadsForm = useForm<{ photos: File[]; audio: File[] }>({ defaultValues: { photos: [], audio: [] } });
    ...
    <PageContainer>
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview" className="space-y-6">
          <Card>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div><div className="text-muted-foreground">Status</div><div><StatusBadge status={ticket.status} /></div></div>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {triageTemplates.map(tmpl => (
                  <button key={tmpl} type="button" className="px-2 py-1 rounded bg-muted hover:bg-secondary text-xs" onClick={()=>{
                  ...
                  {ticket.status === 'waiting_approval' && (
                    <div className="mt-2 flex flex-wrap gap-2 text-xs">
                      <Button size="sm" variant="outline" onClick={async()=>{
                        try {
                          await fetch('/api/notifications/slack', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: `Reminder: approval pending for Ticket ${ticket.ticket_number}` }) });
                          toast.success('Reminder sent');
                        } catch { toast.error('Failed to send reminder'); }
                      }}>Send reminder</Button>
                      <Button size="sm" variant="secondary" onClick={async()=>{ try { await navigator.clipboard.writeText(window.location.href); toast.success('Link copied'); } catch { toast.error('Copy failed'); } }}>Copy link</Button>
                    </div>
                  )}
                </div>
        ...
        <TabsContent value="attachments" className="space-y-6">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-1 text-sm">
                  <button onClick={() => setAttFilter('all')} className={`px-2 py-1 rounded ${attFilter==='all'?'bg-secondary':'hover:bg-muted'}`}>All</button>
                  <button onClick={() => setAttFilter('photo')} className={`px-2 py-1 rounded ${attFilter==='photo'?'bg-secondary':'hover:bg-muted'}`}>Photos</button>
                  <button onClick={() => setAttFilter('audio')} className={`px-2 py-1 rounded ${attFilter==='audio'?'bg-secondary':'hover:bg-muted'}`}>Audio</button>
                </div>
                <div className="ml-auto flex items-center gap-2 text-sm">
                  <span className="text-muted-foreground">Sort</span>
                  <select value={attSort} onChange={(e)=>setAttSort(e.target.value as any)} className="border rounded px-2 py-1 text-sm">
                    <option value="newest">Newest</option>
                    <option value="oldest">Oldest</option>
                    <option value="size">Size</option>
                  </select>
                  <button disabled={selectedIds.size===0} onClick={async()=>{
                    const api = serviceTicketsApi;
                    await Promise.all(Array.from(selectedIds).map(id=>api.deleteTicketAttachment(id)));
                    setSelectedIds(new Set());
                    const listed = await api.listTicketAttachments(ticketId);
                    if (listed.success && listed.data) setAttachments(listed.data);
                  }} className={`px-2 py-1 rounded ${selectedIds.size? 'bg-destructive text-destructive-foreground':'bg-muted text-muted-foreground cursor-not-allowed'}`}>Delete Selected</button>
                </div>
              </div>
              ...
              {attachments.length === 0 ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {attachments
                    .filter(a => attFilter==='all' ? true : attFilter==='photo' ? a.attachment_type==='photo' : a.attachment_type==='audio')
                    .sort((a,b)=>{
                      if (attSort==='size') return (b.file_size||0)-(a.file_size||0);
                      if (attSort==='oldest') return new Date(a.uploaded_at).getTime()-new Date(b.uploaded_at).getTime();
                      return new Date(b.uploaded_at).getTime()-new Date(a.uploaded_at).getTime();
                    })
                    .map((a, idx) => (
                    <div key={a.id} className={`border rounded p-2 text-xs flex flex-col gap-2 relative ${selectedIds.has(a.id)?'ring-2 ring-primary':''}`}>
                      <input type="checkbox" aria-label="Select attachment" className="absolute top-2 left-2" checked={selectedIds.has(a.id)} onChange={(e)=>{
                        const ns = new Set(selectedIds);
                        if (e.target.checked) ns.add(a.id); else ns.delete(a.id);
                        setSelectedIds(ns);
                      }} />
                      <div className="font-medium break-all truncate" title={a.original_name}>{a.original_name}</div>
                      <div className="text-muted-foreground">{a.attachment_type}</div>
                      {a.attachment_type==='photo' && (
                        <img onClick={()=>{ setViewerIndex(idx); setViewerOpen(true); }} src={attUrls[a.id] || a.storage_path} alt={a.original_name} className="w-full h-32 object-cover rounded cursor-zoom-in" />
                      )}
                      {a.attachment_type==='audio' && (
                        <audio src={attUrls[a.id] || a.storage_path} controls className="w-full" />
                      )}
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={() => document.getElementById(`replace-${a.id}`)?.click()}>Replace</Button>
                        <Button variant="destructive" size="sm" onClick={async()=>{ await serviceTicketsApi.deleteTicketAttachment(a.id); const listed = await serviceTicketsApi.listTicketAttachments(ticketId); if (listed.success && listed.data) setAttachments(listed.data); }}>Delete</Button>
                      </div>
                      <input id={`replace-${a.id}`} type="file" className="hidden" onChange={async (e)=>{ const f=e.target.files?.[0]; if(!f) return; await serviceTicketsApi.uploadAttachments({ ticketId, files:[f], type:a.attachment_type }); await serviceTicketsApi.deleteTicketAttachment(a.id); const listed = await serviceTicketsApi.listTicketAttachments(ticketId); if (listed.success && listed.data) setAttachments(listed.data); }} />
                    </div>
                  ))}
                </div>
              )}
          ...
          <MediaViewerModal attachments={attachments.filter(a=>attFilter==='all'?true:attFilter==='photo'?a.attachment_type==='photo':a.attachment_type==='audio')}
            index={viewerIndex} open={viewerOpen} onClose={()=>setViewerOpen(false)} />
        </TabsContent>
            ...
            <CardContent className="space-y-3">
              ...
              {historyLoading ? (
                ...
                <div className="space-y-4">
                  {history
                    .filter(h => activityFilter==='all' ? true : h.action === activityFilter)
                    .map((h) => (
...
function StatusBadge({ status }: { status: ServiceTicket['status'] }) {
  ...
  return <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${cls}`}>{status.replace('_',' ')}</span>;
}
...
function statusClass(status: ServiceTicket['status']) {
  switch (status) {
    ...
    case 'on_hold': return 'bg-zinc-100 text-zinc-700';
    case 'cancelled':
...
function StickyTicketHeader({ ticket, canEdit, onStatusChange }: { ticket: ServiceTicket & { customer?: Customer }; canEdit: boolean; onStatusChange: (s: ServiceTicket['status']) => Promise<void> }) {
    ...
    <div className="sticky top-0 z-30 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="flex items-center justify-between px-2 py-3">
        <div className="flex items-center gap-3">
          ...
          <div className="text-xs text-muted-foreground">Updated {new Date(ticket.updated_at).toLocaleString()}</div>
        </div>
        <div className="flex items-center gap-2">
          {actions.map(a => (
            <Button key={a.label} size="sm" variant={a.variant} disabled={!canEdit} onClick={() => onStatusChange(a.to)}>{a.label}</Button>
          ))}
        </div>
      </div>
    </div>
...
function getActionsForStatus(status: ServiceTicket['status']): { label: string; to: ServiceTicket['status']; variant?: 'default'|'outline'|'secondary'|'destructive' }[] {
  switch (status) {
    case 'reported':
      return [
    ...
    case 'completed':
      return [
        { label: 'Deliver', to: 'delivered', variant: 'default' }
      ];
    case 'on_hold':
      return [
        { label: 'Start', to: 'in_progress', variant: 'default' }
