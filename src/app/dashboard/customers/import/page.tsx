...
function parseCsv(text: string): Row[] {
  ...
  return lines.slice(1).map(line => {
    ...
    return {
      ...
      contact: idx.contact >= 0 ? (cols[idx.contact] || '').trim() : undefined,
      email: idx.email >= 0 ? (cols[idx.email] || '').trim() : undefined,
      address: idx.address >= 0 ? (cols[idx.address] || '').trim() : undefined,
      gst_number: idx.gst_number >= 0 ? (cols[idx.gst_number] || '').trim() : undefined,
    } as Row;
  }).filter(r => r.name);
}
...
export default function CustomersImportPage() {
  ...
  const canImport = useMemo(() => rows.length > 0 && !importing, [rows, importing]);
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="text-sm text-muted-foreground">
              Expected headers: name,contact,email,address,gst_number. One customer per line.
            </div>
            <Textarea value={text} onChange={(e) => setText(e.target.value)} className="min-h-[200px]" placeholder="name,contact,email,address,gst_number\nJohn Doe,9876543210,john@example.com,Main Street 1,GST123..." />
            <div className="flex gap-2">
              <Button type="button" variant="outline" onClick={onParse} disabled={parsing}>Preview</Button>
              <Button type="button" onClick={onImport} disabled={!canImport}>{importing ? 'Importing...' : 'Import'}</Button>
            </div>
          </CardContent>
        </Card>
