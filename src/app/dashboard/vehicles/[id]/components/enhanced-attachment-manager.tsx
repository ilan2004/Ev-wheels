    ...
    async (files: File[]) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          // Upload via API
          const serviceTicketsApi = (await import("@/lib/api/service-tickets")).serviceTicketsApi;
          ...
          if (res.success && res.data) {
            ...
            // Refresh attachments
            const listRes = await serviceTicketsApi.listVehicleAttachments(ticketId, vehicleId);
          ...
          <div className="flex items-center gap-4 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span>•</span>
                <span>{format(new Date(attachment.uploaded_at), "MMM dd, yyyy")}</span>
              </div>
            </div>
    ...
    <>
          ...
          <Tabs value={filterType} onValueChange={(v) => setFilterType(v as any)}>
            <TabsList className="grid w-full grid-cols-4">
              ...
              <TabsTrigger value="photo">
                Photos ({attachments.filter((a) => a.attachment_type === "photo").length})
              </TabsTrigger>
              <TabsTrigger value="audio">
                Audio ({attachments.filter((a) => a.attachment_type === "audio").length})
              </TabsTrigger>
              <TabsTrigger value="document">
                Docs ({attachments.filter((a) => a.attachment_type === "document").length})
              </TabsTrigger>
            </TabsList>
          </Tabs>
      ...
      <Dialog open={showLightbox} onOpenChange={setShowLightbox}>
        <DialogContent className="max-w-4xl">
          ...
          {selectedAttachment && (
            <div className="mt-4">
              {selectedAttachment.attachment_type === "photo" && previewUrls[selectedAttachment.id] && (
                <img
