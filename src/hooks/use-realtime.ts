            ...
            switch (payload.eventType) {
              ...
              case 'UPDATE':
                ...
                if (batteryId) {
                  ...
                  queryClient.invalidateQueries({
                    queryKey: batteryKeys.history(batteryId)
                  });
                }
              ...
              case 'DELETE':
                ...
                if (deletedBatteryId) {
                  ...
                  queryClient.removeQueries({
                    queryKey: batteryKeys.detail(deletedBatteryId)
                  });
                  queryClient.removeQueries({
                    queryKey: batteryKeys.history(deletedBatteryId)
                  });
                }
...
export function useBackgroundSync(
  ...
  const {
    interval = 30000,
    enabled = true,
    tables = ['battery_records', 'customers']
  } = options;
  const queryClient = useQueryClient();
