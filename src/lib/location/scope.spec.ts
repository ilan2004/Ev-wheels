...
global.localStorage = {
  ...
  getItem(key: string) { return (this.store as any).get(key) ?? null; },
  setItem(key: string, value: string) { (this.store as any).set(key, value); },
  removeItem(key: string) { (this.store as any).delete(key); },
  clear() { (this.store as any).clear(); }
} as any;
...
describe('location scoping helpers', () => {
  ...
  it('scopeQuery applies eq filter when active location set', () => {
    ...
    const calls: any[] = [];
    const builder = { eq: (col: string, val: any) => { calls.push([col, val]); return builder; } } as any;
