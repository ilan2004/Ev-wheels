...
class SupabaseVehiclesRepository implements VehiclesApiContract {
  async listVehicles(
    ...
    try {
      ...
      countQuery = scopeQuery('vehicle_cases', countQuery, { isAdmin });
      ...
      if (params.search && params.search.trim()) {
        ...
        countQuery = countQuery.or(
          `vehicle_reg_no.ilike.${term},vehicle_make.ilike.${term},vehicle_model.ilike.${term}`
        );
      }
      ...
      let query = supabase.from('vehicle_cases').select(`
          *,
        ...
        `) as any;
      query = scopeQuery('vehicle_cases', query, { isAdmin });
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Failed to list vehicles',
        data: { vehicles: [], totalCount: 0 }
      };
    }
  }
  ...
  async updateVehicleStatus(
    ...
    try {
      ...
      if (!current.success || !current.data)
        return { success: false, error: 'Vehicle case not found' };
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to update vehicle notes'
      };
    }
  }
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to list vehicle history'
      };
    }
  }
}
