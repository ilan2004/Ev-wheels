...
export interface GlobalSearchResults {
  tickets: Pick<ServiceTicket, 'id' | 'ticket_number' | 'status' | 'vehicle_reg_no' | 'created_at'>[];
  batteries: Pick<BatteryRecord, 'id' | 'serial_number' | 'brand' | 'status'>[];
  customers: { id: string; name: string; contact?: string | null }[];
}

export async function globalSearch(query: string, limits: { tickets?: number; batteries?: number; customers?: number } = {}): Promise<ApiResponse<GlobalSearchResults>> {
  const term = (query || '').trim();
  if (!term) return { success: true, data: { tickets: [], batteries: [], customers: [] } };
