...
export class OptimizedKpiService {
  ...
  async fetchDashboardKpis(): Promise<ApiResponse<OptimizedKpis>> {
    try {
      ...
      let query = supabase.from('dashboard_kpis').select('*');
      ...
      if (error) {
        ...
        console.warn(
          'Dashboard KPIs view not available, falling back to individual queries:',
          error.message
        );
   ...
   */
  async fetchWeeklyDeliveryTrends(
    weeks: number = 8
  ): Promise<ApiResponse<WeeklyDeliveryPoint[]>> {
    try {
      const locationId = getActiveLocationId();
      ...
      if (error) {
        console.warn(
          'Weekly delivery trends view not available, falling back to calculation:',
          error.message
        );
  ...
  async fetchBatterySummaries(
    ...
    try {
      ...
      if (params.search && params.search.trim()) {
        ...
        query = query.or(
          `serial_number.ilike.${term},brand.ilike.${term},customer_name.ilike.${term}`
        );
      }
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to fetch battery summaries'
      };
    }
  }
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to fetch customer summaries'
      };
    }
  }
   ...
   */
  async fetchDashboardBundle(): Promise<
    ApiResponse<{
      kpis: OptimizedKpis;
  ...
  > {
    try {
      ...
      const [kpisResult, trendsResult, batteriesResult, customersResult] =
        await Promise.all([
        ...
        await Promise.all([
          ...
          scopeQuery(
            supabase
              .from('battery_records')
              .select('id', { count: 'exact', head: true })
              .gte('delivered_date', monthStart.toISOString())
   ...
   */
  private async fetchWeeklyDeliveryTrendsLegacy(
    weeks: number
  ): Promise<ApiResponse<WeeklyDeliveryPoint[]>> {
    try {
      const end = new Date();
...
export type {
  OptimizedKpis as Kpis,
  WeeklyDeliveryPoint as WeeklyPoint,
  ApiResponse as KpiResponse
};
