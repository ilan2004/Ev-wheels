...
export interface BillingRepository {
  ...
  listQuotes(filters?: QuoteFilters, page?: number, pageSize?: number): Promise<PaginatedResponse<Quote>>;
  ...
  listInvoices(filters?: InvoiceFilters, page?: number, pageSize?: number): Promise<PaginatedResponse<Invoice>>;
  ...
  // Utilities
  convertQuoteToInvoice(quoteId: string, dueDate: Date, createdBy: string): Promise<Invoice>;
...
export class InMemoryBillingRepository implements BillingRepository {
  ...
  private filterQuotes(quotes: Quote[], filters?: QuoteFilters): Quote[] {
    ...
    return quotes.filter(quote => {
      if (filters.status && !filters.status.includes(quote.status)) return false;
      if (filters.customerName && !quote.customer.name.toLowerCase().includes(filters.customerName.toLowerCase())) return false;
      ...
      if (filters.amountMin && quote.totals.grandTotal < filters.amountMin) return false;
      if (filters.amountMax && quote.totals.grandTotal > filters.amountMax) return false;
      return true;
    });
  }
  
  private filterInvoices(invoices: Invoice[], filters?: InvoiceFilters): Invoice[] {
    if (!filters) return invoices;
    
    return invoices.filter(invoice => {
      if (filters.status && !filters.status.includes(invoice.status)) return false;
      if (filters.customerName && !invoice.customer.name.toLowerCase().includes(filters.customerName.toLowerCase())) return false;
      ...
      if (filters.amountMin && invoice.totals.grandTotal < filters.amountMin) return false;
      if (filters.amountMax && invoice.totals.grandTotal > filters.amountMax) return false;
  ...
  private paginate<T>(items: T[], page: number = 1, pageSize: number = 10): PaginatedResponse<T> {
  ...
  async listQuotes(filters?: QuoteFilters, page: number = 1, pageSize: number = 10): Promise<PaginatedResponse<Quote>> {
    const allQuotes = Array.from(this.quotes.values()).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
  ...
  async updateQuote(id: string, input: UpdateQuoteInput): Promise<Quote> {
    ...
    if (input.items) {
      ...
      updatedTotals = calculateBillingTotals(
        updatedItems,
        input.shippingAmount !== undefined ? input.shippingAmount : existing.totals.shippingAmount || 0,
        input.adjustmentAmount !== undefined ? input.adjustmentAmount : existing.totals.adjustmentAmount || 0
      );
    }
  ...
  async createInvoice(input: CreateInvoiceInput, createdBy: string): Promise<Invoice> {
    ...
    const number = generateInvoiceNumber(DEFAULT_INVOICE_CONFIG, existingNumbers);
  ...
  async listInvoices(filters?: InvoiceFilters, page: number = 1, pageSize: number = 10): Promise<PaginatedResponse<Invoice>> {
    const allInvoices = Array.from(this.invoices.values()).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
  ...
  async updateInvoice(id: string, input: UpdateInvoiceInput): Promise<Invoice> {
    ...
    if (input.items) {
      ...
      updatedBalanceDue = calculateBalanceDue(updatedTotals.grandTotal, paymentsTotal);
    }
  ...
  async deleteInvoice(id: string): Promise<void> {
    ...
    const paymentsToDelete = Array.from(this.payments.values()).filter(p => p.invoiceId === id);
  ...
  async addPayment(input: CreatePaymentInput, createdBy: string): Promise<Payment> {
    ...
    const totalPayments = existingPayments.reduce((sum, p) => sum + p.amount, 0) + input.amount;
    const newBalance = calculateBalanceDue(invoice.totals.grandTotal, totalPayments);
  ...
  seed(quotes: Quote[] = [], invoices: Invoice[] = [], payments: Payment[] = []): void {
    this.quotes.clear();
    this.invoices.clear();
    this.payments.clear();
