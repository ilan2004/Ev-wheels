...
export function InvoiceForm({
  ...
  useEffect(() => {
    ...
    setValue('items', formItems as any, {
      shouldDirty: true,
  ...
  const handleInvalidSubmit = (errors: any) => {
    ...
    const firstError = (() => {
      ...
      if (errors?.customer?.name?.message)
        return String(errors.customer.name.message);
      ...
      if (Array.isArray(errors?.items)) {
        for (const e of errors.items) {
          if (e?.description?.message)
            return `Line item: ${String(e.description.message)}`;
        }
      }
      // Fallback generic
