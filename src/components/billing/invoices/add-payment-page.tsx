...
import { CreatePaymentFormData } from '@/lib/billing/schemas';
...
export function AddPaymentPage({ invoiceId }: AddPaymentPageProps) {
  ...
  const handleSubmit = async (data: CreatePaymentFormData) => {
    ...
    try {
      setLoading(true);
      await billingRepository.addPayment(data, user.id);
