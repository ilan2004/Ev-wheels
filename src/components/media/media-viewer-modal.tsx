...
export function MediaViewerModal({
  ...
  const isImage =
    att.attachment_type === 'photo' || att.mime_type.startsWith('image/');
  const isAudio =
    att.attachment_type === 'audio' || att.mime_type.startsWith('audio/');
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className='max-w-5xl'>
        ...
        <div className='bg-muted relative flex min-h-[420px] items-center justify-center overflow-hidden rounded'>
          {isImage && (
            <img
              src={
                signedUrl ||
                (att as any).preview_url ||
