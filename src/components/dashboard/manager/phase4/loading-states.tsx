...
export function LoadingSpinner({ 
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center gap-3", className)}>
      <motion.div
        className={cn("border-2 border-muted rounded-full border-t-primary", sizeClasses[size])}
        animate={{ rotate: 360 }}
...
export function ErrorState({
  ...
  const getIcon = () => {
    switch (type) {
      case 'network':
        return <IconWifi className="h-12 w-12 text-muted-foreground/50" />;
      case 'notFound':
        return <IconBulb className="h-12 w-12 text-muted-foreground/50" />;
      case 'unauthorized':
        return <IconAlertTriangle className="h-12 w-12 text-destructive/50" />;
      default:
        return <IconAlertTriangle className="h-12 w-12 text-muted-foreground/50" />;
    }
  };
