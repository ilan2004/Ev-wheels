...
export function EnhancedTicketCreation({
  ...
  const [technicians, setTechnicians] =
    useState<Technician[]>(MOCK_TECHNICIANS);
  const [selectedTemplate, setSelectedTemplate] =
    useState<SymptomTemplate | null>(null);
  ...
  useEffect(() => {
    if (watchedValues.symptom || watchedValues.description) {
      const text =
        `${watchedValues.symptom} ${watchedValues.description}`.toLowerCase();
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      toast.success(
        `Job card ${result.data.ticket_number} created successfully!`,
        {
          description: selectedTemplate
            ? `Using template: ${selectedTemplate.symptom}`
            : undefined
        }
      );
                                ...
                                >
                                  <div className='flex flex-col'>
                                    ...
                                    <div className='text-muted-foreground text-sm'>
                                      {customer.phone} •{' '}
                                      {customer.service_history_count} previous
                                      services
                                    </div>
                                    {customer.recent_vehicles?.[0] && (
                                      <div className='text-muted-foreground text-xs'>
                                        Last: {customer.recent_vehicles[0].make}{' '}
                                        {customer.recent_vehicles[0].model}
                                      </div>
                                    )}
                                  </div>
                            ...
                            >
                              <div>
                                ...
                                <div className='text-xs opacity-75'>
                                  {PRIORITY_LABELS[template.suggestedPriority]}{' '}
