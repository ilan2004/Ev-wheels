...
export function FrontDeskDashboard({ user }: { user: SerializedUser }) {
  ...
  const rankedTopCustomers = React.useMemo(() => {
    ...
    const scored = items.map((i) => {
      ...
      const score =
        0.6 * revenueScore + 0.3 * activityScore + 0.1 * recencyScore;
  ...
  const [defaultCustomersLoading, setDefaultCustomersLoading] =
    ...
    <ScrollArea className='h-[calc(100dvh-52px)]'>
      <div className='flex flex-1 flex-col p-4 sm:p-6 lg:p-8'>
        <div className='flex flex-col gap-6'>
          ...
          <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
            <div>
              <h1 className='text-2xl font-bold tracking-tight'>
                Hi Front Desk Manager, {userName}
              </h1>
              <p className='text-muted-foreground mt-1 text-sm'>
                Find customers and job cards quickly. Create a new job card in
                seconds.
              </p>
            </div>
                    ...
                    <CardTitle className='text-sm font-medium'>
                      ...
                      <Badge variant='secondary'>
                        {hasQuery
                          ...
                          : defaultCustomers.length || topCustomers.length}
                      </Badge>
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                          ...
                          {defaultCustomers.slice(0, 10).map((c) => {
                            ...
                            return (
                              <li key={c.id}>
                                ...
                                >
                                  <span
                                    className={`absolute top-0 bottom-0 left-0 w-1 ${v.accent} rounded-l`}
                                  ></span>
                                  <div className='flex items-center justify-between gap-2 text-sm'>
                                    ...
                                    <div className='text-primary/70 opacity-0 transition-opacity group-hover:opacity-100'>
                                      Open →
                                    </div>
                                  </div>
                                </Link>
          ...
          {/* Connected Cases (Both Vehicle & Battery) */}
          <Card>
            <CardHeader>
          ...
          <Card>
            <CardContent className='text-muted-foreground py-4 text-sm'>
              Tip: To create a job card, search/select the customer, then click
              &ldquo;New Job Card&rdquo;. You can also directly use the New Job
              Card button above.
            </CardContent>
          </Card>
        </div>
...
function ConnectedCasesWidget() {
  const [connectedCases, setConnectedCases] = React.useState<
    Array<{
      ticketId: string;
      ticketNumber: string;
      vehicleCase?: { id: string; status: string; regNo: string };
      batteryCase?: { id: string; status: string; serial: string };
      createdAt: string;
    }>
  >([]);
  const [loading, setLoading] = React.useState(false);
  ...
  React.useEffect(() => {
    const loadConnectedCases = async () => {
      setLoading(true);
      try {
        // Get recent tickets with both vehicle_case_id and battery_case_id
        const ticketsRes = await serviceTicketsApi.listTickets({ limit: 50 });
        if (!ticketsRes.success || !ticketsRes.data) return;

        const bothCaseTickets = ticketsRes.data.filter(
          (t) => t.vehicle_case_id && t.battery_case_id
        );

        const connectedCasesData = await Promise.all(
          bothCaseTickets.slice(0, 5).map(async (ticket) => {
            const caseData: any = {
              ticketId: ticket.id,
            ...
            if (ticket.vehicle_case_id) {
              try {
                const vehicleRes = await vehiclesApi.fetchVehicle(
                  ticket.vehicle_case_id
                );
                if (vehicleRes.success && vehicleRes.data) {
                  caseData.vehicleCase = {
                    id: vehicleRes.data.id,
                    status: vehicleRes.data.status,
                    regNo: vehicleRes.data.vehicle_reg_no || 'No Reg'
                  };
                }
              } catch (error) {
                console.error('Error loading vehicle case:', error);
              }
            }
            ...
            if (ticket.battery_case_id) {
              try {
                const batteryRes = await batteryApi.fetchBattery(
                  ticket.battery_case_id
                );
                if (batteryRes.success && batteryRes.data) {
                  caseData.batteryCase = {
                    id: batteryRes.data.id,
                    status: batteryRes.data.status,
                    serial: batteryRes.data.serial_number.slice(0, 8) + '...'
                  };
                }
              } catch (error) {
                console.error('Error loading battery case:', error);
              }
            }
        ...
        setConnectedCases(
          connectedCasesData.filter((c) => c.vehicleCase || c.batteryCase)
        );
      } catch (error) {
        console.error('Error loading connected cases:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (connectedCases.length === 0) {
    return (
      <div className='text-muted-foreground py-4 text-sm'>
        No recent tickets with both Vehicle and Battery cases.
      </div>
    );
  }
  ...
  return (
    <div className='space-y-3'>
      {connectedCases.map((connectedCase) => (
        <div
        ...
        >
          <div className='flex items-center justify-between'>
            <Link
              href={`/dashboard/job-cards/${connectedCase.ticketId}`}
              className='text-sm font-medium hover:underline'
            >
              {connectedCase.ticketNumber}
            </Link>
            <div className='text-muted-foreground text-xs'>
              {new Date(connectedCase.createdAt).toLocaleDateString('en-GB', {
                day: '2-digit',
                month: 'short'
              })}
            </div>
          ...
          <div className='flex items-center gap-4 text-xs'>
            {connectedCase.vehicleCase && (
              ...
              >
                ...
                <Badge
                  variant='outline'
                  className={`text-xs ${getStatusColor(connectedCase.vehicleCase.status)}`}
                >
                  {connectedCase.vehicleCase.status.replace('_', ' ')}
                </Badge>
                <span className='text-muted-foreground'>
                  ({connectedCase.vehicleCase.regNo})
                </span>
              </Link>
            )}
            ...
            {connectedCase.batteryCase && (
              ...
              >
                ...
                <Badge
                  variant='outline'
                  className={`text-xs ${getStatusColor(connectedCase.batteryCase.status)}`}
                >
                  {connectedCase.batteryCase.status.replace('_', ' ')}
                </Badge>
                <span className='text-muted-foreground'>
      ...
      <div className='border-t pt-2'>
        <Button asChild variant='ghost' size='sm' className='w-full'>
          <Link href='/dashboard/job-cards'>View All Job Cards</Link>
        </Button>
      </div>
    </div>
  );
