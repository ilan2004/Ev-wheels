...
type Ticket = { id: string; ticket_number: string; symptom: string; status: string; due_date?: string | null; priority?: number | null; pipeline_order?: number | null };

type Buckets = Record<string, Ticket[]>;

export function ManagerKanban() {
  const [buckets, setBuckets] = React.useState<Buckets>({ reported: [], triaged: [], in_progress: [], completed: [] });
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const fetched: Buckets = { reported: [], triaged: [], in_progress: [], completed: [] };
      for (const st of statuses) {
        let query = supabase
          .from('service_tickets')
          .select('id, ticket_number, symptom, status, due_date, priority, pipeline_order')
          .eq('status', st)
    ...
    try {
      ...
      setBuckets(prev => {
        ...
        for (const key of Object.keys(next)) {
          next[key] = next[key].map(t => t.id === ticketId ? { ...t, priority: p } : t);
        }
        return next;
      });
      ...
      toast.error('Failed to update priority', { description: e?.message || 'Please try again.' });
      load();
    }
  };
  ...
  const onDragEnd = async (event: DragEndEvent) => {
    ...
    const fromId = (active.data?.current as any)?.status || (Object.keys(buckets).find(k => buckets[k].some(t => t.id === activeId)) as string | undefined);
    let toId = (over.data?.current as any)?.status as string | undefined;
    if (!toId) {
      // If over a ticket item, infer its column
      toId = Object.keys(buckets).find(k => buckets[k].some(t => t.id === overId)) as string | undefined;
    }
    ...
    let dstIdx = overId === toId ? 0 : dstListPre.findIndex(t => t.id === overId);
    ...
    const computeNewOrder = (list: Ticket[], index: number): number => {
      ...
      if (index === 0) {
        ...
        return (Number.isFinite(here as any) ? Math.max(here as number, base + 1) : base + 1);
      }
      if (index === list.length - 1) {
        const last = list[index - 1]?.pipeline_order;
        const base = Number.isFinite(last as any) ? (last as number) : 0;
        return (Number.isFinite(here as any) ? Math.min(here as number, base - 1) : base - 1);
      }
    ...
    try {
      const payload: any = fromId === toId ? { pipeline_order: newOrder } : { status: toId, pipeline_order: newOrder };
      const { error } = await supabase
        .from('service_tickets')
        .update(payload)
        .eq('id', activeId);
      ...
      toast.error('Failed to update ticket', { description: e?.message || 'Please try again.' });
      load();
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <Input placeholder="Search tickets (number or symptom)" value={search} onChange={(e) => setSearch(e.target.value)} className="max-w-md" />
      ...
      <DndContext collisionDetection={closestCenter} onDragEnd={onDragEnd}>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {COLUMNS.map(col => (
            <KanbanColumn key={col.id} id={col.id} title={col.title} count={buckets[col.id].length}>
              <SortableContext items={buckets[col.id].map(t => t.id)} strategy={verticalListSortingStrategy}>
          ...
          {buckets[col.id].map((t) => (
                    <SortableTicket
                      ...
                      ticket={t}
                      status={col.id}
                      onDueDate={() => setDueModal({ open: true, ticketId: t.id, due: t.due_date || null })}
...
function SortableTicket({ ticket, status, onDueDate, onPriorityChange }: { ticket: Ticket; status: string; onDueDate: () => void; onPriorityChange: (p: 1 | 2 | 3) => void }) {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: ticket.id, data: { status } });
  ...
  return (
    <li ref={setNodeRef} style={style} className="rounded border bg-card p-2" {...attributes} {...listeners}>
      <div className="flex items-center justify-between gap-2">
        <div className="text-sm font-medium truncate">{ticket.ticket_number} — {ticket.symptom}</div>
        <div className="flex items-center gap-2 shrink-0">
          <span className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded border ${colorClasses}`}>{label}</span>
          <Select value={String(p)} onValueChange={(val) => onPriorityChange(Number(val) as 1 | 2 | 3)}>
      ...
      <div className="flex gap-2 mt-2">
        <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); onDueDate(); }}>Due Date</Button>
...
function KanbanColumn({ id, title, count, children }: { id: string; title: string; count: number; children: React.ReactNode }) {
  ...
  return (
    <div ref={setNodeRef} className={`rounded-lg border p-3 ${isOver ? 'bg-muted/50' : 'bg-background/50'}`}>
