...
const CATEGORY_CONFIG = {
  core: {
    ...
    items: ['Dashboard', 'Search', 'Activity'],
  ...
  workflow: {
    ...
    items: ['Vehicles', 'Batteries', 'Job Cards'],
...
export default function AppSidebar() {
  ...
  const categorizedNavItems = React.useMemo(() => {
    ...
    const categorized = filteredNavItems.reduce(
      (acc, item) => {
    ...
    return Object.entries(categorized).sort(
  ...
  return (
    <Sidebar collapsible={isMobile ? 'offcanvas' : 'none'}>
      <SidebarHeader>
        ...
        <div className='border-sidebar-border/50 flex items-center gap-3 border-b px-4 py-3'>
          <div className='relative'>
            <company.logo className='text-primary h-7 w-7' />
            <div className='border-sidebar-background absolute -top-1 -right-1 h-3 w-3 animate-pulse rounded-full border-2 bg-green-500' />
          </div>
                  ...
                  {user && (
                    <UserAvatarProfile
                      ...
                      user={{
                        imageUrl: '',
                        fullName:
                          `${(user as any).user_metadata?.firstName ?? ''} ${(user as any).user_metadata?.lastName ?? ''}`.trim() ||
                          null,
