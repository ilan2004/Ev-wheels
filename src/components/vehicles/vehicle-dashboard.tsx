...
export function VehicleDashboard({ onFilterSelect, className }: VehicleDashboardProps) {
  ...
  const getTrendDisplay = (trend: 'up' | 'down' | 'neutral', change: number) => {
  ...
  return (
    <div className={className}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Vehicle Analytics</h2>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 mb-6">
        <MetricCard
          ...
          change={metrics ? {
            value: getTrendDisplay(metrics.totalVehicles.trend, metrics.totalVehicles.change).text,
        ...
        <MetricCard
          ...
          value={metrics ? `${metrics.averageTurnaroundTime.current.toFixed(1)} days` : '0 days'}
          change={metrics ? {
            value: getTrendDisplay(metrics.averageTurnaroundTime.trend, metrics.averageTurnaroundTime.change).text,
        ...
        <MetricCard
          ...
          change={metrics ? {
            value: getTrendDisplay(metrics.todaysArrivals.trend, metrics.todaysArrivals.change).text,
          ...
          variant="elevated"
          actionable
          onClick={() => onFilterSelect?.({ dateFrom: new Date().toISOString().split('T')[0] })}
        />
        ...
        <MetricCard
          ...
          change={metrics ? {
            value: getTrendDisplay(metrics.readyForPickup.trend, metrics.readyForPickup.change).text,
          ...
          variant="elevated"
          actionable
          onClick={() => onFilterSelect?.({ status: ['completed', 'ready_for_pickup'] })}
        />
        ...
        <MetricCard
          ...
          loading={loading}
          variant={metrics?.overdueCases.severity === 'critical' ? 'danger' : 'elevated'}
          actionable
          onClick={() => {
            const sevenDaysAgo = new Date();
            sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
            onFilterSelect?.({ dateTo: sevenDaysAgo.toISOString().split('T')[0] });
          }}
        />

        <MetricCard
          title="Technician Util."
          value={metrics ? `${metrics.technicianUtilization.current.toFixed(0)}%` : '0%'}
          change={metrics ? {
            value: getTrendDisplay(metrics.technicianUtilization.trend, metrics.technicianUtilization.change).text,
      ...
      <div className="grid gap-6 md:grid-cols-2 mb-6">
        ...
        <EnhancedCard variant="elevated" animated>
          <CardHeader>
            ...
            <CardDescription>
              Daily vehicle arrivals over time
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={analytics.intakeTrends}>
                    ...
                    <ChartTooltip 
                      ...
                      labelFormatter={(date) => formatDashboardDate(date, 'short')}
                    />
                    <Line
                      type="monotone"
                      dataKey="count"
        ...
        <EnhancedCard variant="elevated" animated>
          ...
          <CardContent>
            {loading ? (
              ...
              >
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={analytics.technicianWorkload} layout="horizontal">
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis type="number" />
                ...
                {analytics.serviceTimeAnalysis.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                    ...
                    <div className="text-right">
                      <span className="font-semibold">{item.averageHours.toFixed(1)}h</span>
                    </div>
                  </div>
                ))}
              </div>
