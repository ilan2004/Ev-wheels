...
function getCardStyle(status: VehicleStatus, isUrgent: boolean, isCritical: boolean) {
  const baseClasses = "relative overflow-hidden transition-all duration-300 group";
  
  const statusStyles: Record<VehicleStatus, string> = {
    received: "bg-gradient-to-br from-blue-50 to-blue-100/50 border-blue-200 hover:border-blue-400",
    diagnosed: "bg-gradient-to-br from-purple-50 to-purple-100/50 border-purple-200 hover:border-purple-400",
    in_progress: "bg-gradient-to-br from-amber-50 to-yellow-100/50 border-yellow-300 hover:border-yellow-500",
    completed: "bg-gradient-to-br from-emerald-50 to-green-100/50 border-green-300 hover:border-green-500",
    delivered: "bg-gradient-to-br from-gray-50 to-gray-100/50 border-gray-300 hover:border-gray-400",
    on_hold: "bg-gradient-to-br from-red-50 to-orange-100/50 border-red-200 hover:border-red-400",
    cancelled: "bg-gradient-to-br from-gray-100 to-gray-200/50 border-gray-400 hover:border-gray-500",
  };
  ...
  return cn(baseClasses, statusStyles[status] || statusStyles.received, urgencyClass);
}

// Priority indicator based on days
function getPriorityLevel(days: number): { level: 'low' | 'medium' | 'high' | 'critical', color: string, icon?: React.ComponentType<{ className?: string }> } {
  if (days > 10) return { level: 'critical', color: 'text-red-600 bg-red-100', icon: AlertTriangle };
  if (days > 7) return { level: 'high', color: 'text-orange-600 bg-orange-100', icon: AlertCircle };
  if (days > 3) return { level: 'medium', color: 'text-yellow-600 bg-yellow-100', icon: Clock };
  ...
  return (
    <Card 
      className={cn(
        ...
        isDragging && "opacity-70 rotate-3 scale-110 shadow-2xl z-50"
      )}
    >
      ...
      <div className="relative space-y-3 p-3">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger>
                  <Badge 
                    variant={priority.level === 'critical' ? 'destructive' : priority.level === 'high' ? 'default' : 'secondary'}
                    className={cn(
                      "h-5 px-2 text-xs font-semibold flex items-center gap-1",
                      priority.color
                    )}
                  >
        ...
        <div className="relative h-24 bg-gradient-to-br from-muted/30 to-muted/50 rounded-lg overflow-hidden shadow-inner">
          ...
          <div className="absolute bottom-1 right-1 bg-background/90 backdrop-blur-sm px-1.5 py-0.5 rounded text-[10px] font-mono">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide">Assigned to</span>
            {vehicle.technician ? (
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger>
                    <div className="flex items-center gap-1.5 bg-background/50 rounded-full px-2 py-1">
                  ...
                  <TooltipContent>
                    ...
                    <p className="text-xs text-muted-foreground">{vehicle.technician.email}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ) : (
              <Badge variant="outline" className="text-[10px] h-5 px-2 bg-yellow-50 border-yellow-200 text-yellow-700">
          ...
          {vehicle.last_activity_at && (
            <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
              ...
              <span>Last activity: {new Date(vehicle.last_activity_at).toLocaleDateString()}</span>
            </div>
          )}
        </div>
        ...
        {vehicle.technician_notes && (
          <div className="mt-2 p-2 bg-muted/50 rounded-md">
            <p className="text-[11px] text-muted-foreground line-clamp-2">
              <span className="font-medium">Note:</span> {vehicle.technician_notes}
            </p>
          </div>
        )}
